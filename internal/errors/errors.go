...
// the rest of agent-dd keeps the internal/errors import path while the
...
type (
	...
	// APIError is the family name for the shared output.Error type. It still
	// carries WithHint/WithCause (and now WithHints/WithRetryAfter) for chaining.
	APIError = out.Error
)
