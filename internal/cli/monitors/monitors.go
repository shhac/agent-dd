...
func Register(root *cobra.Command, globals func() *shared.GlobalFlags) {
	...
	registerCreate(mon, globals)
	registerUpdate(mon, globals)
	registerDelete(mon, globals)
	registerMute(mon, globals)
	registerUnmute(mon, globals)
	registerUsage(mon)

	libcli.HandleUnknownCommand(mon, "run 'agent-dd monitors usage' to see the available subcommands")
	root.AddCommand(mon)
}
