...
// monitorFixtures returns a freshly constructed fixture set. It is a function
// rather than a package var because monitors are writable — each server needs
// its own copy, and returning new literals keeps the value types intact in a
// way a generic deep-clone would not (a JSON round-trip would turn every `id`
// into a float64 and silently break ID lookups).
func monitorFixtures() []map[string]any {
	return []map[string]any{
		{"id": 1001, "name": "High CPU on checkout-service", "type": "metric alert", "overall_state": "Alert", "query": "avg(last_5m):avg:system.cpu.user{service:checkout} > 90", "message": "CPU usage is critically high on checkout-service", "tags": []string{"service:checkout", "env:production", "team:platform"}, "muted": false, "priority": 1, "last_triggered_ts": 1711893600, "created": "2025-11-01T08:00:00Z", "modified": "2026-03-15T14:30:00Z", "options": map[string]any{"thresholds": map[string]any{"critical": 90.0, "warning": 80.0}, "notify_audit": false, "require_full_window": false, "renotify_interval": 240, "include_tags": true, "on_missing_data": "default", "new_group_delay": 0}},
...
// incidentFixtures is a function for the same reason as monitorFixtures:
// incidents are writable, so each server needs its own set.
func incidentFixtures() []map[string]any {
	return []map[string]any{
