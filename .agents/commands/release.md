...
You are performing a release of the `agent-dd` CLI (Go version). Follow these steps exactly.
...
```bash
rm -rf dist/
GOOS=darwin GOARCH=arm64 go build -ldflags="-s -w -X main.version=${new_version}" -o "dist/agent-dd-darwin-arm64" ./cmd/agent-dd
GOOS=darwin GOARCH=amd64 go build -ldflags="-s -w -X main.version=${new_version}" -o "dist/agent-dd-darwin-amd64" ./cmd/agent-dd
GOOS=linux GOARCH=amd64 go build -ldflags="-s -w -X main.version=${new_version}" -o "dist/agent-dd-linux-amd64" ./cmd/agent-dd
GOOS=linux GOARCH=arm64 go build -ldflags="-s -w -X main.version=${new_version}" -o "dist/agent-dd-linux-arm64" ./cmd/agent-dd
GOOS=windows GOARCH=amd64 go build -ldflags="-s -w -X main.version=${new_version}" -o "dist/agent-dd-windows-amd64.exe" ./cmd/agent-dd

# Create tarballs
cd dist
for bin in agent-dd-darwin-arm64 agent-dd-darwin-amd64 agent-dd-linux-amd64 agent-dd-linux-arm64; do
  tar czf "${bin}.tar.gz" "$bin"
done
shasum -a 256 *.tar.gz agent-dd-windows-amd64.exe > checksums-sha256.txt
cd ..
```
...
```bash
./dist/agent-dd-darwin-arm64 --version
./dist/agent-dd-darwin-arm64 usage
```
...
gh release create "v${new_version}" dist/*.tar.gz dist/agent-dd-windows-amd64.exe dist/checksums-sha256.txt \
  --title "v${new_version}" \
  --notes "$notes"
```
...
```bash
ls ../homebrew-tap/Formula/agent-dd.rb
```
...
- All `agent-sql` references → `agent-dd`
- Version, URLs, and SHA256 values
- Test: assert_match version and "Datadog" in help output

**If it exists:** Read checksums from `dist/checksums-sha256.txt` and update the formula:

1. Read `../homebrew-tap/Formula/agent-dd.rb`
...
4. Ensure the `install` block installs shell completions — after `bin.install`, add
   `generate_completions_from_executable(bin/"agent-dd", "completion")` (the binary supports `completion bash|zsh|fish`)
5. Commit and push:
   ```bash
   cd ../homebrew-tap
   git add Formula/agent-dd.rb
   git commit -m "agent-dd ${new_version}"
   git push
   cd -
   ```

**IMPORTANT:** Always `cd` back to the agent-dd repo after updating the tap.
...
- `brew install shhac/tap/agent-dd` command for new users
- `brew upgrade shhac/tap/agent-dd` command for existing users
